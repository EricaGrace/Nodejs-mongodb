...
```
db.salles.find({"avis":{$size:1}},{"nom":true,"_id":true})
```
...
```
db.salles.find({ "avis.date": { $gt: new Date('2019-11-15') } }, { "nom": true, "_id": false})
```
...
db.salles.find(
    ...
    ,{ "nom": true, "capacite": true, "_id": false }
    )
```
...
```
db.salles.find({$where: "this.smac == true && this.styles.length > 2"}, {"nom":true, "_id":false})
```

=> non testé (MongoServerError: $where not allowed in this atlas tier)
...
```
db.salles.updateMany({"nom": {$regex: /[^aeiou]+$/}}, {$push: {avis: {date: new Date(), note:10}}})
```
...
```
db.salles.updateOne(
   {"_id": {$eq:ObjectId("6496e83fdc97107ffaa47385")}},
   { "$set": { capacite: 60 },
     "$unset":{smac:""}
    }
)
